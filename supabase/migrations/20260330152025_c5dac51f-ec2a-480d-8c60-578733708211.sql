...
-- Create role enum
CREATE TYPE public.app_role AS ENUM ('user', 'owner', 'developer', 'admin');
...
CREATE POLICY "Admins can update all roles" ON public.user_roles FOR UPDATE TO authenticated USING (public.has_role(auth.uid(), 'admin'));
